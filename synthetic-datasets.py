...
# -------------------- DATASET 2: PHARMACEUTICAL SALES --------------------
...
# -------------------- DATASET 3: WEARABLE HEALTH DATA --------------------
...
# -------------------- DATASET 4: WASTEWATER SURVEILLANCE DATA --------------------
...
def generate_all_datasets(save_to_csv=True):
    ...
    if save_to_csv:
        ...
        patient_df.to_csv('datasets/patient_records.csv', index=False)
        pharma_df.to_csv('datasets/pharmaceutical_sales.csv', index=False)
        wearable_df.to_csv('datasets/wearable_health_data.csv', index=False)
        wastewater_df.to_csv('datasets/wastewater_surveillance.csv', index=False)
...
def create_exploratory_visualizations(datasets):
    """Create exploratory visualizations for the generated datasets"""
    
    # ---- Patient Records Visualizations ----
    
    # Age distribution
    plt.figure(figsize=(10, 6))
    sns.histplot(datasets['patient_records']['Age'], kde=True, bins=20)
    ...
    diagnosis_counts = datasets['patient_records']['Diagnosis'].value_counts()
    ...
    plt.tight_layout()
    plt.savefig('datasets/diagnosis_distribution.png')
    
    # Patient admissions over time
    plt.figure(figsize=(14, 6))
    admission_ts = datasets['patient_records']['Timestamp'].value_counts().sort_index()
    admission_ts.plot()
    plt.title('Patient Admissions Over Time')
    plt.xlabel('Date')
    plt.ylabel('Number of Admissions')
    plt.tight_layout()
    plt.savefig('datasets/admissions_timeseries.png')
    
    # ---- Pharmaceutical Sales Visualizations ----
    
    # Sales of key medications over time
    key_meds = ['Tamiflu', 'Paracetamol', 'Azithromycin', 'Paxlovid']
    plt.figure(figsize=(14, 8))
    
    for med in key_meds:
        med_data = datasets['pharmaceutical_sales'][datasets['pharmaceutical_sales']['Medication'] == med]
        plt.plot(med_data['Date'], med_data['Units_Sold'], label=med)
    
    plt.title('Sales of Key Medications Over Time')
    plt.xlabel('Date')
    plt.ylabel('Units Sold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('datasets/pharma_sales_timeseries.png')
    
    # ---- Wearable Health Data Visualizations ----
    
    # Fever detection over time
    plt.figure(figsize=(14, 6))
    fever_ts = datasets['wearable_health_data'].groupby(datasets['wearable_health_data']['Timestamp'].dt.date)['Fever_Detected'].mean()
    fever_ts.plot()
    plt.title('Proportion of Fever Detections Over Time')
    plt.xlabel('Date')
    plt.ylabel('Proportion with Fever')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('datasets/fever_detection_timeseries.png')
    
    # Temperature distribution by fever status
    plt.figure(figsize=(10, 6))
    sns.histplot(
        data=datasets['wearable_health_data'], 
        x='Body_Temperature', 
        hue='Fever_Detected',
        multiple='stack',
        kde=True
    )
    plt.title('Body Temperature Distribution by Fever Status')
    plt.xlabel('Body Temperature (°C)')
    plt.ylabel('Count')
    plt.tight_layout()
    plt.savefig('datasets/temp_distribution_by_fever.png')
    
    # ---- Wastewater Surveillance Visualizations ----
    
    # Pathogen concentration over time for SARS-CoV-2
    plt.figure(figsize=(14, 6))
    covid_data = datasets['wastewater_surveillance'][
        datasets['wastewater_surveillance']['Pathogen'] == 'SARS-CoV-2'
    ]
    
    for location in covid_data['Location'].unique():
        location_data = covid_data[covid_data['Location'] == location]
        plt.plot(location_data['Date'], location_data['Concentration_copies_per_mL'], label=location)
    
    plt.title('SARS-CoV-2 Concentration in Wastewater Over Time')
    plt.xlabel('Date')
    plt.ylabel('Concentration (copies/mL)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('datasets/covid_wastewater_timeseries.png')
    
    print("Exploratory visualizations created and saved to datasets folder")

# Create visualizations
create_exploratory_visualizations(datasets)
